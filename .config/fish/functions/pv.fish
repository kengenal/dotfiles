function pv --description 'Open project with nvim'
    set project (find ~/Desktop/work ~/Desktop/python ~/Projects ~/.hyperdots ~/.dotfiles -maxdepth 1 -type d -not -path "*/.git" -not -path "*/.idea" -not -path "*/.venv" 2>/dev/null | fzf --ignore-case --preview "ls --color=always -a1 {}")
    if test -n "$project"
        cd "$project"
